...
If Not %OSARCH% == 64BIT (
 Cls & Echo You must have a x64 architecture to run THE BIG ONE's Windows 10 Optimization Script.
 Goto END
)
...
cls
:
echo				THE BIG ONE's WINDOWS 10 OPTIMIZIATION SCRIPT v12.7
echo				===================================================
echo.
:STEP1
echo.
...
start /WAIT "Open-Shell" "%WorkDir%\Open-Shell\OpenShellSetup_4_4_191.exe" /qn
...
REM Always Show Menus in Windows Explorer:
Reg.exe add "HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" /v "AlwaysShowMenus" /t REG_DWORD /d "1" /f > nul
...
start /WAIT "AppUninstall" powershell.exe -ExecutionPolicy Bypass -NoLogo -NonInteractive -Command "Get-AppxPackage -AllUsers | where-object {$_.name -notlike %winstore%} | where-object {$_.name -notlike %netflix%} | where-object {$_.name -notlike %wincalc%} | where-object {$_.name -notlike %NetFramework%} | where-object {$_.name -notlike %VCLibs%} | where-object {$_.name -notlike %Xbox%} | where-object {$_.name -notlike %Terminal%} | where-object {$_.name -notlike %nVidia%} | where-object {$_.name -notlike %Notepad%} | Remove-AppxPackage"
...
start /WAIT "AppUninstall" powershell.exe -ExecutionPolicy Bypass -NoLogo -NonInteractive -Command "Get-AppxProvisionedPackage -online | where-object {$_.packagename -notlike %winstore%} | where-object {$_.packagename -notlike %netflix%} | where-object {$_.packagename -notlike %wincalc%} | where-object {$_.packagename -notlike %NetFramework%} | where-object {$_.packagename -notlike %VCLibs%} | where-object {$_.packagename -notlike %Xbox%} | where-object {$_.packagename -notlike %Terminal%} | where-object {$_.packagename -notlike %nVidia%} | where-object {$_.packagename -notlike %Notepad%} | Remove-AppxProvisionedPackage -online"
...
"%WorkDir%\Tools\NSudoLC.exe" -U:T -wait -ShowWindowMode:Hide Reg.exe add "HKLM\SYSTEM\CurrentControlSet\Services\WinDefend" /v "Start" /t REG_DWORD /d "4" /f
...
:OS.Settings.Import 
REM echo Registry Key:			%KEY_NAME%
REM echo Registry Value:			%ValueName%	%ValueType%	%ValueValue%
